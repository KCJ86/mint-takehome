---
title: "Troubleshooting - v2.0"
description: "Common issues and solutions for Salsa as a Service v2.0"
---

## Quick Diagnostics

Before diving into specific issues, run these quick checks:

<Steps>
  <Step title="Check System Status">
    Visit our [status page](https://status.salsaasaservice.com) to see if there
    are any ongoing service issues.
  </Step>
  <Step title="Verify Internet Connection">
    Ensure you have a stable internet connection and can access other websites.
  </Step>
  <Step title="Clear Browser Cache">
    Clear your browser cache and cookies, then try logging in again.
  </Step>
  <Step title="Check Browser Compatibility">
    Ensure you're using a supported browser (Chrome, Firefox, Safari, Edge).
  </Step>
</Steps>

---

## Common Issues

<CardGroup cols={3}>
  <Card title="Login & Authentication" icon="lock" href="#login-issues">
    Troubleshoot login issues, password problems, and 2FA setup.
  </Card>
  <Card title="Data Synchronization" icon="arrows-rotate" href="#data-sync">
    Resolve POS integration and inventory sync problems.
  </Card>
  <Card title="Production Issues" icon="industry" href="#production-issues">
    Fix quality control and batch tracking problems.
  </Card>
  <Card title="Mobile App Issues" icon="mobile" href="#mobile-issues">
    Resolve app crashes, freezes, and offline mode problems.
  </Card>
  <Card title="Performance Issues" icon="gauge" href="#performance-issues">
    Fix slow loading times and browser performance problems.
  </Card>
  <Card title="API & Advanced" icon="code" href="#advanced-troubleshooting">
    Resolve API authentication and data format issues.
  </Card>
</CardGroup>

---

## Login and Authentication

### Login Issues

**Problem**: Unable to log into your account

<Steps>
  <Step title="Verify Credentials">
    Double-check your email address and password. Ensure Caps Lock is off.
  </Step>
  <Step title="Reset Password">
    Use the "Forgot Password" link on the login page to reset your password.
  </Step>
  <Step title="Check Email">
    Look for account verification emails in your spam folder.
  </Step>
  <Step title="Contact Support">
    If the issue persists, contact support with your account email address.
  </Step>
</Steps>

<Warning>
  **Security Note**: Never share your password with anyone. Our support team
  will never ask for your password.
</Warning>

### Two-Factor Authentication

**Problem**: Issues with 2FA codes or setup

- **Code Not Working**: Ensure your device's time is synchronized
- **Lost Device**: Use backup codes or contact support
- **App Issues**: Try a different authenticator app (Google Authenticator, Authy)
- **Setup Problems**: Follow our [2FA setup guide](/guides/two-factor-auth)

---

## Data Synchronization

### POS Integration Issues

**Problem**: POS data not syncing with Salsa as a Service

<Tabs>
  <Tab title="Square">
    <Steps>
      <Step title="Check Square Connection">
        Verify the Square integration is still connected in your dashboard.
      </Step>
      <Step title="Reauthorize Integration">
        Disconnect and reconnect your Square account.
      </Step>
      <Step title="Check API Permissions">
        Ensure Square API permissions include inventory management.
      </Step>
    </Steps>
  </Tab>
  <Tab title="Toast">
    <Steps>
      <Step title="Verify Toast API Key">
        Check that your Toast API credentials are valid and not expired.
      </Step>
      <Step title="Check Menu Mapping">
        Ensure salsa menu items are properly mapped to inventory items.
      </Step>
    </Steps>
  </Tab>
  <Tab title="Clover">
    <Steps>
      <Step title="Update Clover App">
        Ensure the Salsa as a Service app is updated in your Clover dashboard.
      </Step>
      <Step title="Check Permissions">
        Verify the app has necessary permissions for data access.
      </Step>
    </Steps>
  </Tab>
  <Tab title="General">
    <Steps>
      <Step title="Check Network Connection">
        Ensure stable internet connection for both systems.
      </Step>
      <Step title="Review Sync Logs">
        Check the integration logs for error messages.
      </Step>
      <Step title="Test Manual Sync">
        Try manually triggering a data sync from your dashboard.
      </Step>
    </Steps>
  </Tab>
</Tabs>

### Inventory Sync Problems

**Problem**: Inventory levels not updating correctly

**Common Causes:**

- Network connectivity issues
- API rate limiting
- Incorrect unit conversions
- Data validation errors

<Steps>
  <Step title="Check Sync Status">
    Navigate to **Settings > Integrations** and check the sync status.
  </Step>
  <Step title="Manual Sync">
    Try manually syncing inventory data from the integrations page.
  </Step>
  <Step title="Review Error Logs">
    Check the error logs for specific sync failures.
  </Step>
  <Step title="Contact Support">
    If issues persist, contact support with your sync logs.
  </Step>
</Steps>

---

## Production Issues

### Quality Control Problems

**Problem**: Quality scores not recording correctly

- **Check Device Calibration**: Ensure quality testing devices are properly calibrated
- **Review Testing Procedures**: Verify staff are following correct testing protocols
- **Update Software**: Ensure the mobile app is updated to the latest version
- **Check Permissions**: Verify staff have proper permissions for quality testing

### Batch Tracking Issues

**Problem**: Production batches not tracking properly

<Note>
  **Batch Requirements**: Each batch must have a unique batch number and valid
  recipe assignment.
</Note>

<Steps>
  <Step title="Verify Batch Data">
    Check that all required batch information is entered correctly.
  </Step>
  <Step title="Check Recipe Assignment">
    Ensure the batch is assigned to a valid recipe.
  </Step>
  <Step title="Review Staff Permissions">
    Verify the assigned chef has proper permissions for batch management.
  </Step>
</Steps>

---

## Mobile App Issues

### App Crashes

**Problem**: Mobile app crashes or freezes

<Steps>
  <Step title="Update App">
    Ensure you're running the latest version of the Salsa as a Service mobile
    app.
  </Step>
  <Step title="Restart App">Force close the app and restart it.</Step>
  <Step title="Clear App Data">
    Clear app cache and data (you'll need to log in again).
  </Step>
  <Step title="Reinstall App">
    Uninstall and reinstall the app from your device's app store.
  </Step>
</Steps>

### Offline Mode Issues

**Problem**: App not working in offline mode

- **Check Offline Data**: Ensure critical data is downloaded for offline use
- **Sync Before Going Offline**: Manually sync data before losing internet connection
- **Check Storage Space**: Ensure sufficient device storage for offline data
- **Update App**: Use the latest app version with improved offline capabilities

---

## Performance Issues

### Slow Loading Times

**Problem**: Dashboard or app loading slowly

<CardGroup cols={3}>
  <Card title="Browser Issues" icon="globe" href="#browser-performance">
    Clear cache, disable extensions, try a different browser.
  </Card>
  <Card title="Network Issues" icon="wifi" href="#network-performance">
    Check internet speed, try a different network.
  </Card>
  <Card title="Data Issues" icon="database" href="#data-performance">
    Reduce data load, optimize queries.
  </Card>
</CardGroup>

### Browser Performance

<Steps>
  <Step title="Clear Browser Cache">
    Clear cookies, cache, and browsing data.
  </Step>
  <Step title="Disable Extensions">
    Temporarily disable browser extensions to identify conflicts.
  </Step>
  <Step title="Try Different Browser">
    Test with a different browser to isolate the issue.
  </Step>
  <Step title="Update Browser">
    Ensure you're using the latest browser version.
  </Step>
</Steps>

---

## Advanced Troubleshooting

### API Integration Issues

#### Authentication Errors

**Problem**: API requests returning 401 Unauthorized

- **Check API Key**: Verify the API key is correct and not expired
- **Check Permissions**: Ensure the API key has necessary permissions
- **Check Rate Limits**: Verify you haven't exceeded rate limits
- **Check Request Format**: Ensure requests are properly formatted

#### Data Format Issues

**Problem**: API requests failing due to data format errors

**Common Issues:**

- Incorrect date formats
- Invalid data types
- Missing required fields
- Invalid JSON syntax

```javascript
// Example: Proper API request format
const requestData = {
  name: "House Salsa",
  category: "salsa",
  batch_size: 5.0, // Use numbers, not strings
  production_date: "2024-01-15T08:00:00Z", // ISO 8601 format
  ingredients: [
    {
      ingredient_id: "ing_123",
      quantity: 2.0,
      unit: "lbs",
    },
  ],
};
```

### Database Issues

#### Data Inconsistencies

**Problem**: Data not matching between systems

- **Check Sync Logs**: Review integration logs for sync errors
- **Manual Data Verification**: Compare data between systems manually
- **Reset Integration**: Disconnect and reconnect integrations
- **Contact Support**: Provide specific examples of data inconsistencies

---

## Getting Help

### Self-Service Resources

<CardGroup cols={3}>
  <Card
    title="Knowledge Base"
    icon="book"
    href="https://help.salsaasaservice.com"
  >
    Search our comprehensive knowledge base for solutions.
  </Card>
  <Card
    title="Video Tutorials"
    icon="video"
    href="https://salsaasaservice.com/tutorials"
  >
    Watch step-by-step video guides for common tasks.
  </Card>
  <Card
    title="Community Forum"
    icon="users"
    href="https://community.salsaasaservice.com"
  >
    Get help from other users and share solutions.
  </Card>
</CardGroup>

### Contact Support

When contacting support, please include:

<Steps>
  <Step title="Account Information">
    Your account email and restaurant name.
  </Step>
  <Step title="Issue Description">
    A detailed description of the problem you're experiencing.
  </Step>
  <Step title="Steps Taken">
    What troubleshooting steps you've already tried.
  </Step>
  <Step title="Error Messages">Any error messages or codes you're seeing.</Step>
  <Step title="Screenshots">Screenshots of the issue if applicable.</Step>
  <Step title="Browser/Device Info">
    Your browser version and device information.
  </Step>
</Steps>

<CardGroup cols={2}>
  <Card
    title="Email Support"
    icon="envelope"
    href="mailto:support@salsaasaservice.com"
  >
    support@salsaasaservice.com
  </Card>
  <Card title="Live Chat" icon="message" href="#">
    Available directly in your dashboard.
  </Card>
  <Card title="Phone Support" icon="phone" href="tel:1-800-SALSA-HELP">
    1-800-SALSA-HELP (Mon-Fri, 9 AM - 6 PM EST)
  </Card>
  <Card title="Support Ticket" icon="ticket" href="#">
    Create a ticket directly in your dashboard.
  </Card>
</CardGroup>

<Note>
  **Response Times**: We typically respond to support requests within 2-4 hours
  during business hours.
</Note>

### Emergency Support

For critical issues affecting your restaurant operations:

- 🚨 **Emergency Hotline**: 1-800-SALSA-911
- 📱 **Text Support**: Text "HELP" to SALSA-911
- 💬 **Priority Chat**: Available for Enterprise customers

<Warning>
  **Emergency Support**: Available 24/7 for Enterprise customers. Standard
  support hours apply for other plans.
</Warning>
